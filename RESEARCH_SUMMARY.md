...
2. **Green-List practitioners** must either shrink the effective
   vocabulary (top-k on a smaller subset) or accept a two-tier
   ecosystem where the watermark only works on legacy tokenizers.
3. **Watermark designers** should treat vocabulary size as a
   first-class parameter in future statistical watermark schemes.
